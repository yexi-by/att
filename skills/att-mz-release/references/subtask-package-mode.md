...
交叉验证：
用 context/source.json 和至少一个上下文文件互相核对关键译名；报告选中译名依据、疑难项依据和仍需主代理确认的风险。
...
交叉验证：
每个选中路径至少用字段名、相邻字段、参数值形态或内部字符串叶子候选中的两个证据互相核对；空结果要说明检查范围和空结果依据。
...
5. 对照 `context/` 内结构化数据抽查关键条目和交叉验证摘要，确认没有编造路径、遗漏明显玩家可见文本、误选资源、脚本、公式或内部字段。
