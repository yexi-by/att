...
- 漏翻：先运行 `translation-status`、`quality-report` 和必要时的 `audit-active-runtime`，确认是否还有没成功保存译文的文本、未纳入规则的文本来源或当前运行文件残留。
...
- 译文缺口：继续翻译或导出手动填写译文表。
- 写入缺口：重新检查覆盖审计和写回流程。
- 插件源码硬编码：用 `scan-plugin-source-text` 生成翻译源风险报告；怀疑写回后的当前运行文件有漏翻、坏控制符或 JS 语法错误时，用 `audit-active-runtime` 直接审计当前文件。需要处理文本时，先让用户确认，再按插件源码支线导出 AST 地图。
