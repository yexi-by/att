...
| --- | --- | --- | --- | --- | --- |
| 启动与注册 | 确认发行版、游戏、工作区、源语言和模型配置可用 | `<发行版目录>`、`<游戏目录>`、源语言、`<工作区>` | 已注册 `<游戏标题>` 与可用工作区 | `doctor`、`add-game` 和游戏检查成功 | 游戏结构无效、源语言未确认、工作区不可写、模型配置缺失且本轮需要翻译 |
...
1. 在 `<发行版目录>` 运行静态检查，确认 CLI 可启动；注册游戏时显式传入 `--source-language ja` 或 `--source-language en`。
...
7. 插件源码文本是少见支线；普通流程不自动读取插件源码做语义分析，也不把当前运行审计里的源语言字符串直接当成漏翻清单。若 `plugin-source-risk-report.json` 为高风险，或试玩反馈明确指向 `js/plugins/*.js` 硬编码文本，必须先询问用户；用户肯定后读取插件源码任务契约，导出 AST 地图，必要时只读对应插件源码做语义交叉验证，再把每个活跃源语言候选归入翻译 selector 或排除 selector 并导入插件源码规则；用户未确认或仍有未归类 selector 时停止正文翻译。
